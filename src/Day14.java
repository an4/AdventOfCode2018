 ...
 * the digits of the sum of the current recipes' scores. With the current recipes' scores of 3 and
 * 7, their sum is 10, and so two new recipes would be created: the first with score 1 and the
 * second with score 0. If the current recipes' scores were 2 and 3, the sum, 5, would only create
 * one recipe (with a score of 5) with its single digit.
 *
 ...
 * this, the Elf steps forward through the scoreboard a number of recipes equal to 1 plus the score
 * of their current recipe. So, after the first round, the first Elf moves forward 1 + 3 = 4 times,
 * while the second Elf moves forward 1 + 7 = 8 times. If they run out of recipes, they loop back
 * around to the beginning. After the first round, both Elves happen to loop around until they land
 * on the same recipe that they had in the beginning; in general, they will move to different
 * recipes.
 *
...
public class Day14 {
    ...
    private static final int INPUT = 360781;

    private final static String SEQUENCE = "360781";
    ...
    private static Recipe getRecipeWithRank(Recipe list, int rank) {
    ...
    private static Recipe addAtTheEndOfList(Recipe list, Recipe recipe) {
        Recipe lastRecipe = getRecipeWithRank(list, recipe.rank - 1);
        Recipe firstRecipe = lastRecipe.next;
        lastRecipe.next = recipe;
        recipe.next = firstRecipe;
        return recipe;
    }

    private static void findNthRecipe() {
        int rank = 0;
        Recipe list = new Recipe(rank++, 3);
        addAtTheEndOfList(list, new Recipe(rank++, 7));

        Recipe workerOne = list;
        Recipe workerTwo = list.next;

        while (rank < INPUT + 10) {
            int score = workerOne.value + workerTwo.value;
            // Generate new recipes
            if (score > 9) {
                Recipe one = new Recipe(rank++, score / 10);
                Recipe two = new Recipe(rank++, score % 10);
                addAtTheEndOfList(list, one);
                addAtTheEndOfList(list, two);
            } else {
                Recipe one = new Recipe(rank++, score);
                addAtTheEndOfList(list, one);
            }
            // Assign new recipes to the workers
            int moveWorkerOneBy = workerOne.value + 1;
            for (int i = 0; i < moveWorkerOneBy; i++) {
                workerOne = workerOne.next;
            }
            int moveWorkerTwoBy = workerTwo.value + 1;
            for (int i = 0; i < moveWorkerTwoBy; i++) {
                workerTwo = workerTwo.next;
            }
        }

        Recipe best = getRecipeWithRank(list, INPUT);
        for (int i = 0; i < 10; i++) {
            System.out.print(best.value);
            best = best.next;
        }
    }

    private static void findSequencePosition() {
        int rank = 0;
        Recipe list = new Recipe(rank++, 3);
        Recipe end = addAtTheEndOfList(list, new Recipe(rank++, 7));
        ...
        while (true) {
            ...
            if (score > 9) {
                Recipe one = new Recipe(rank++, score / 10);
                Recipe two = new Recipe(rank++, score % 10);
                end = addAtTheEndOfList(end, one);
                end = addAtTheEndOfList(end, two);
                sequence += one.value;
                sequence += two.value;
            } else {
                Recipe one = new Recipe(rank++, score);
            ...
            while (sequence.length() > SEQUENCE.length()) {
                if (sequence.substring(0, SEQUENCE.length()).equals(SEQUENCE)) {
                    System.out.println(rank - sequence.length());
                    return;
                }

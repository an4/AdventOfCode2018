...
public class Day09 {
    private static final int NUM_PLAYERS = 458;
//    private static final int LAST_MARBLE = 71307;
    private static final int LAST_MARBLE = 7130700;
    ...
    private static Marble placeMarbleAndReturn(Marble newMarble, Marble currentMarble) {
    ...
    private static Map.Entry<Marble, Integer> playSpecialMarbleMove(Marble currentMarble) {
        Marble marbleToRemove = currentMarble;
        for (int i = 0; i < 7; i++) {
            marbleToRemove = marbleToRemove.prev;
        }
        Marble marbleToReturn = marbleToRemove.next;
        marbleToRemove.prev.next = marbleToRemove.next;
        marbleToRemove.next.prev = marbleToRemove.prev;
        return new AbstractMap.SimpleEntry<>(marbleToReturn, marbleToRemove.score);
    }

    private static long getMaxScore(long[] scores) {
    ...
    private static void prinMarble(Marble start) {
        int startId = start.score;
        do {
            System.out.println(start.score + " ");
            start = start.next;
        } while (start.score != startId);
        System.out.println();
    }
    ...
    private static void solution() {
        ...
        while (nextMarble <= LAST_MARBLE) {
            if (nextMarble % 23 == 0) {
                Map.Entry<Marble, Integer> result = playSpecialMarbleMove(currentMarble);
                currentMarble = result.getKey();
                playerScores[currentPlayer] += (result.getValue() + nextMarble);
            } else {
                currentMarble = placeMarbleAndReturn(new Marble(nextMarble), currentMarble);
            }
            nextMarble++;
        ...
        System.out.println(getMaxScore(playerScores));
    }
